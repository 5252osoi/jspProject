...
-- unique key name을 추가하시오.
alter table hakkwa add unique(name);

insert into hakkwa value (101,'컴퓨터공학과');
insert into hakkwa value (102,'멀티미디어과');
insert into hakkwa value (103,'경영학과');
insert into hakkwa value (104,'생물학과');
insert into hakkwa value (105,'기계공학과');
insert into hakkwa value (106,'영문학과');
insert into hakkwa value (107,'일문학과');

delete from hakkwa where code = 106;
delete from hakkwa where code = 101;

update hakkwa set code = 105 where code = 101;
/*이미 student 테이블에서 쓰고있기때문에 못바꿈*/

update hakkwa set code = 107 where code = 103;
update hakkwa set code = 105 where code = 107;
...
create table student (
	st_code int not null auto_increment primary key,	/*학생고유번호*/
	name varchar(20) not null,
	hakkwa_code int,
	age int default 20,
	foreign key(hakkwa_code) references hakkwa(code)
	on update cascade 			/*관련된 정보가 바뀌면 같이 바뀌게함*/
	on delete set null			/*다른곳에서 자료가 지워지면 null값으로 바뀌게함*/
);
desc student;
...
insert into student values (default,'이연습',107,default); /*107번 필드값이 없어서 안만들어짐*/
...
--일반조인(Inner Join) : 각 테이블에는 서로 매칭이 되는 필드가 포함되어있어야한다.
select * from hakkwa;
select * from student;
...
--LEFT JOIN*(왼쪽기준) : 왼쪽자료를 모두 가져오고 오른쪽은 만족하는것만 가져온다. 없으면 NULL로 채운다.
select * from hakkwa h left join student s on h.code = s.hakkwa_code;

--RIGHT JOIN(오른쪽기준) : 오른쪽자료를 모두가져오고  왼쪽은 만족하는것만 가져온다.  없으면 NULL로 채운다.
...
/*합집합:Full outer Join(Mysql에는 없음;) Mysql에서는 UNION명령어를 사용한다.*/
