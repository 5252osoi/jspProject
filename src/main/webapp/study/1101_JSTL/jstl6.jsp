...
%>		
		...
		<div>
			...
			3-2.문자열 추출(subString()) : <%=str.substring(5) %> / <%-- ${fn:substring(str, 5)} 이렇게는 안됨 --%>
																${fn:substring(str,5,fn:length(str))} /
																${fn:substring(str,5,-1)} <%-- 뒤에서 첫번째 위치(-1) --%>
																											<br/>
			<hr/>
			4-1.특정문자의 위치값(indexOf()) 'o'의 위치 : <%=str.indexOf("o") %> /${fn:indexOf(str,'o')}<br/>
			4-2.특정문자의 위치값(lastIndexOf()) 'o'의 위치 : <%=str.lastIndexOf("o") %> <%-- /${fn:lastIndexOf(str,'o')} 이렇게안됨--%><br/>
			...
			4-4. str변수의 'o'문자의 2번째 위치값만 출력하기 (마지막'o'문자의 위치값?)<br/>
			~~~~~~~~~출력???~~~~~~~~<br/>
			<hr/>
			...
			~~~~~~~~2번째 'o' 위치값을 기준으로 출력 => <br/>
			
			<hr/>
			6.문자열분리(split(변수,분리할문자)) : 문자분리 후에는 분리된 문자들을 변수(배열)에 담아줘야한다.<br/>
			...
			7.문자치환 (replace()) <br/>
			예) str변수 안의 'o'를 'O'로 치환 . . . <br/>
			<%=str.replace("o","O") %> / ${fn:replace(str,'o','O')} <br/>
			<hr/>
			
			8.특정문자(열)의 포함 유무 (contains())<br/>
			str : ${str}<br/>
			예)str 변수 안에 "My" 가 포함되어있는지 확인 => ${fn:contains(str,'My')}<br/>
			예)str 변수 안에 "my" 가 포함되어있는지 확인 => ${fn:contains(str,'my')}<br/>
