...
<div class="container">
  ...
  <p>사용법1 :<br/>
<%--     <c:forEach var="i" begin="1" end="10" step="1"> step="1"일 때는 생략할 수 있다 --%>
  ...
  <p>사용법2 :<br/>
<%
	String[] cards = {"국민","BC","현대","삼성","롯데","농협","신한"};
...
%>
  ...
  <br/>
  -문제1 : 모든카드를 출력하되, 국민카드는 파란색, 삼성카드는 빨간색으로 출력하시오.<br/>
  	<c:forEach var="card" items="${cards}">
  ...
  -문제3: 모든카드를 출력하되, 첫번째 카드는 빨간색, 마지막 카드는 파란색으로 출력하시오.<br/>
  	...
  	<c:forEach var="card" items="${cards}" varStatus="st">
  		<c:choose>
  			...
  			<c:when test="${!st.first && !st.last}">${card},</c:when>
  		</c:choose>
  	</c:forEach>
  	<br/><br/>
  -문제4: 구구단 연습 . . .<br/>
